...
import { Home, Trophy, UserCheck, Image, Gamepad2, MessageCircleHeart } from "lucide-react";
...
export default function Navbar() {
  ...
  return (
    <>
      {/* Top bar (Desktop + Mobile Logo/Register) */}
          ...
          {navItems.map((item) => {
            ...
            return (
              <Link
                key={item.to}
                to={item.to}
                className={`flex-shrink-0 flex-1 min-w-[3.5rem] flex flex-col items-center gap-1 px-1 py-1.5 rounded-lg text-[10px] font-bold transition-colors ${
